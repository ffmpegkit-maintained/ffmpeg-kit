...
enum AVPictureType forced_kf_apply(void *logctx, KeyframeForceCtx *kf,
                         ...
                         AVFrame *next_picture)
{
    ...
    ost->kf.dropped_keyframe = ost->last_dropped && next_picture && !!(next_picture->flags & AV_FRAME_FLAG_KEY);
...
static void print_report(int is_last_report, int64_t timer_start, int64_t cur_time)
{
    ...
    for (OutputStream *ost = ost_iter(NULL); ost; ost = ost_iter(ost)) {
        ...
        q = enc ? ost->quality / (float) FF_QP2LAMBDA : -1;

        if (local_print_stats && vid && ost->st->codecpar->codec_type == AVMEDIA_TYPE_VIDEO) {
        ...
        if (!vid && ost->st->codecpar->codec_type == AVMEDIA_TYPE_VIDEO) {
            ...
            if (local_print_stats && is_last_report)
                av_bprintf(&buf, "L");
            if (qp_hist) {
                int j;
                int qp = lrintf(q);
            ...
            if (local_print_stats && enc && (enc->flags & AV_CODEC_FLAG_PSNR) &&
...
void ffmpeg_var_cleanup() {
    ...
    first_report = 1;
    for(int i = 0; i < FF_ARRAY_ELEMS(qp_histogram); i++) {
        qp_histogram[i] = 0;
    }
}
