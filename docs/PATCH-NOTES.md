...
- A `--full` attempt eventually hung for ~6h until GitHub's own hard timeout killed it (conclusion: cancelled, no error — confirmed via direct API check on `run_started_at`/`updated_at`). Root cause turned out to be `fftools_ffmpeg_mux_init.c` assigning the literal `1` to several 1-bit signed bitfields (truncates to `-1`, harmless but now clang-flagged) during the **always-run** `ndk-build` step for `libffmpegkit.so` itself — reached only after every other enabled library finishes, which is why it took so long to surface and looked like a silent hang rather than a fast compile error. Confirmed and fixed via an isolated `--enable-libass`-only test build (reaches that same `ndk-build` step in ~10 minutes without needing the other ~25 libraries `--full` enables, instead of waiting hours): added `-Wno-error=single-bit-bitfield-constant-conversion` to `MY_CFLAGS` in `android/jni/Android.mk`.
